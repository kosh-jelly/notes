...
# Also copy any referenced images/attachments from published notes
echo ""
echo "Syncing attachments..."

# Find image references in published content and copy them
find "$QUARTZ_CONTENT" -name "*.md" -type f | while read -r mdfile; do
    # Extract image references like ![[image.png]] or ![](path/to/image.png)
    grep -oE '\!\[\[([^]]+)\]\]|\!\[([^]]*)\]\(([^)]+)\)' "$mdfile" 2>/dev/null | while read -r match; do
        # Extract filename from wikilink or markdown link
        if [[ "$match" =~ \!\[\[([^]]+)\]\] ]]; then
